...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.find('?') >= 0:
            ...
            args = self.query(
                cgi.parse_qs(self.path[self.path.find('?') + 1:]))
        else:
            page = self.path[1:]
            args = []
...
div#pages a {color:black;text-decoration:none;-moz-border-radius:10px;
...
"""
    ...
    def index(self, a="0", p="0"):
        ...
        if MyHandler.win.cb_albums.get_active():
            nom = data.albums[a]
            ll = data.getAlbum(a)
            hmenu = "".join(["<a class='%s' href='index?a=%d&p=0'>%s</a>" %
                            (a == data.albums.index(i) and "sel" or "",
                            data.albums.index(i), i) for i in data.albums])
        else:
            nom = "Selection"
            ll = data.getAlbum()
            hmenu = ""
        ...
        if len(ll) > NB:
            hpages = "Pages: " + "".join(
                ["<a class='%s' href='index?a=%d&p=%d'>%d</a>" %
                 (p == i and "sel" or "", a, i, i + 1) for i in range(nbp)])
        else:
            hpages = ""
        hphotos = "".join(
            ["<div><a href='view?n=%d'><img src='thumb?n=%d'></a>%s</div>" %
             (all.index(n), all.index(n), cd2rd(n.date))
             for n in ll[p * NB:(p + 1) * NB]])
