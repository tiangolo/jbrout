...
from gtk     import gdk
from gobject import signal_new, TYPE_INT, TYPE_LONG, TYPE_BOOLEAN, TYPE_PYOBJECT, TYPE_NONE, SIGNAL_RUN_LAST
...
# Custom signals
signal_new('extlistview-dnd', gtk.TreeView, SIGNAL_RUN_LAST, TYPE_NONE, (gdk.DragContext, TYPE_INT, TYPE_INT, gtk.SelectionData, TYPE_INT, TYPE_LONG))
...
class ExtListView(gtk.TreeView):
    ...
    def __init__(self, columns, sortable=True, dndTargets=[], useMarkup=False):
        """
            If sortable is True, the user can click on headers to sort the content of the list
    ...
    def __cmpRows(self, row1, row2, criteria, ascending):
        ...
        for index, criterion in enumerate(criteria):
            ...
            if ascending or index != 0: result = cmp(row1[criterion], row2[criterion])
            else:                       result = cmp(row2[criterion], row1[criterion])
    ...
    def onButtonReleased(self, tree, event):
        ...
        if stateClear and event.state & gtk.gdk.BUTTON1_MASK and self.getSelectedRowsCount() > 1:
            pathInfo = self.get_path_at_pos(int(event.x), int(event.y))
    ...
    def onMouseMotion(self, tree, event):
        ...
        if self.dndContext is None and self.drag_check_threshold(self.dndStartPos[0], self.dndStartPos[1], int(event.x), int(event.y)):
            self.dndContext = self.drag_begin([DND_INTERNAL_TARGET], gtk.gdk.ACTION_COPY, 1, event)
