...
def splitFile(ffile):
   ffile = ffile.replace("\\","/")
   if ffile[-1:]=="/": ffile = ffile[:-1]
   sdir = os.path.dirname(ffile)+"/"
   sfile = os.path.basename(ffile)
   return sdir,sfile
...
# ============================================================================================
   ...
   def normalizeName(file):
        ...
        return _Command.normalizeName(file)
   ...
   def __read(self):
      ...
      self.comment = re.sub( u"%.*%", u"",  t["jpegcomment"] )

      self.tags = self._extractTags( t["jpegcomment"] )
   ...
   def _extractTags(self,buf):
      mo = re.findall( "%.*%", buf )

      if mo:
         tags = mo[0]
         self.start = string.find(buf,tags)
         self.end = self.start + len(tags)
   ...
   def _write(self):
      ...
      buf = self.comment + msg

      if _Command.setJpegComment(self.file,buf):
